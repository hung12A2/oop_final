...
public class InfoAuctionRestAssured extends BaseRestAssuredClass{

	private static String[] listUnitTest = Constant.API_INFO_AUCTION;
    ...
    public static ResponseUnitTest UnitTest000() {
		...
		try {
			Response res = given().header("Authorization", Constant.ACCESS_TOKEN)
	                ...
	                .when()
	                .get(RequestApiInfoAuction.apiPath + "/" + RequestApiInfoAuction.UnitTest000.auctionId);
			rut.setName(listUnitTest[0]);
	        rut.setOutput(res.asPrettyString());
			
			InfoAuctionResponseModel resObj = mapper.readValue(res.asString(), InfoAuctionResponseModel.class);
